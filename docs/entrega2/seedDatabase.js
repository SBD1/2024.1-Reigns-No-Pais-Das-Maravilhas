...
console.log('hostseed:', process.env.DB_HOST);
console.log('userseed:', process.env.DB_USER);
console.log('passwordseed:', process.env.DB_PASSWORD);
console.log('databaseseed:', process.env.DB_NAME);
console.log('portseedseed:', process.env.DB_PORT);
...
const seedDatabase = async () => {
  ...
  try {
    ...
    await pool.query(
      ...
      ['o enfeite da rainha', 'A Rainha te ama tanto que decidiu fazer de sua cabeça uma coroa. Agora, estarão juntas para sempre.', insertImage('../game/images/deaths/flow.png')]
    );
    await pool.query(
      'INSERT INTO Ending (title, description, image) VALUES ($1, $2, $3)',
      ['sem coração', 'Seu coração pertence à Rainha. Ela o guardou em um lugar melhor que o seu corpo.', insertImage('../game/images/deaths/flow.png')]
    );
    await pool.query(
      'INSERT INTO Ending (title, description, image) VALUES ($1, $2, $3)',
      ['a descabeçada', 'Sua cabeça é um belo ornamento. A Rainha a colocou ao lado do seu trono.', insertImage('../game/images/deaths/flow.png')]
    );
    await pool.query(
      'INSERT INTO Ending (title, description, image) VALUES ($1, $2, $3)',
      ['a prisioneira', 'Você desafiou a Rainha Vermelha, e ela garantiu que você nunca mais fosse vista.', insertImage('../game/images/deaths/hanged.png')]
    );
    await pool.query(
        'INSERT INTO Ending (title, description, image) VALUES ($1, $2, $3)',
        ['a exilada', 'A Rainha Vermelha te baniu do País das Maravilhas, jogando-a no desfiladeiro e ao esquecimento.', insertImage('../game/images/deaths/hanged.png')]
    );
    await pool.query(
        'INSERT INTO Ending (title, description, image) VALUES ($1, $2, $3)',
        ['inimiga da coroa', 'Você se tornou inimiga da Rainha Vermelha e pagou o preço com a vida.', insertImage('../game/images/deaths/hanged.png')]
    );
    await pool.query(
        'INSERT INTO Ending (title, description, image) VALUES ($1, $2, $3)',
        ['aclamada e condenada', 'O povo te aclamou como heroína, mas a Rainha Vermelha não tolera rivais.', insertImage('../game/images/deaths/guillotine.png')]
    );
    await pool.query(
        'INSERT INTO Ending (title, description, image) VALUES ($1, $2, $3)',
        ['rainha usurpadora', 'O povo quis te coroar, mas a Rainha Vermelha garantiu que isso nunca acontecesse.', insertImage('../game/images/deaths/atacada.png')]
    );
    await pool.query(
        'INSERT INTO Ending (title, description, image) VALUES ($1, $2, $3)',
        ['a rainha que nunca foi', 'O povo te considerou a nova rainha, mas não é assim que as coisas funcionam... Você foi atacada.', insertImage('../game/images/deaths/atacada.png')]
    );
    await pool.query(
        'INSERT INTO Ending (title, description, image) VALUES ($1, $2, $3)',
        ['a banida', 'Rejeitada pelo povo, você foi expulsa e lançada no Abismo do Esquecimento.', insertImage('../game/images/deaths/selva.png')]
    );
    await pool.query(
        'INSERT INTO Ending (title, description, image) VALUES ($1, $2, $3)',
        ['forasteira solitária', 'Sem o apoio do povo, você foi deixada à própria sorte e desapareceu.', insertImage('../game/images/deaths/sozinha.png')]
    );
    await pool.query(
        'INSERT INTO Ending (title, description, image) VALUES ($1, $2, $3)',
        ['ostracizada', 'Detestada por todos, você foi banida e lançada à selva.', insertImage('../game/images/deaths/selva.png')]
    );
    await pool.query(
        'INSERT INTO Ending (title, description, image) VALUES ($1, $2, $3)',
        ['prisioneira da loucura', 'Você foi internada após ser consumida pela loucura do País das Maravilhas.', insertImage('../game/images/deaths/acordada.png')]
    );
    await pool.query(
        'INSERT INTO Ending (title, description, image) VALUES ($1, $2, $3)',
        ['refém dos delírios', 'Você acordou do surto, mas, perdida em suas fantasias, foi internada para sempre.', insertImage('../game/images/deaths/acordada.png')]
    );
    await pool.query(
        'INSERT INTO Ending (title, description, image) VALUES ($1, $2, $3)',
        ['consumida pela ilusão', 'Convencida de que estava em um mundo de fantasia, você foi internada.', insertImage('../game/images/deaths/acordada.png')]
    );
    await pool.query(
        'INSERT INTO Ending (title, description, image) VALUES ($1, $2, $3)',
        ['a doente', 'Com imunidade baixa, você foi vítima da Gripe das Maravilhas.', insertImage('../game/images/deaths/doente.png')]
    );
    await pool.query(
        'INSERT INTO Ending (title, description, image) VALUES ($1, $2, $3)',
        ['de porcelana', 'Seus ossos eram frágeis como porcelana. Você caiu e se quebrou toda.', insertImage('../game/images/deaths/osso.png')]
    );
    await pool.query(
        'INSERT INTO Ending (title, description, image) VALUES ($1, $2, $3)',
        ['a desnutrida', 'Você se alimentou mal, só tomando porções e biscoitos. Eventualmente, seu corpo parou de funcionar.', insertImage('../game/images/deaths/doente.png')]
    );

    console.log('Populando tabela Resource');
    ...
    console.log('Populando tabela Scene');

    // População da tabela Scene  
        await pool.query(
          'INSERT INTO Scene (scene_id, name, image) VALUES ($1, $2, $3)',
          [1, 'Gato', insertImage('../game/images/scene/gato.png')]
        );
        await pool.query(
          'INSERT INTO Scene (scene_id, name, image) VALUES ($1, $2, $3)',
          [2, 'Chapeleiro Maluco', insertImage('../game/images/scene/gato.png')]
        );
        await pool.query(
          'INSERT INTO Scene (scene_id, name, image) VALUES ($1, $2, $3)',
          [3, 'Coelho Branco', insertImage('../game/images/scene/coelho.png')]
        );
        await pool.query(
          'INSERT INTO Scene (scene_id, name, image) VALUES ($1, $2, $3)',
          [4, 'Rainha de Copas', insertImage('../game/images/scene/rainha.png')]
        );
        await pool.query(
          'INSERT INTO Scene (scene_id, name, image) VALUES ($1, $2, $3)',
          [5, 'Lebre de Março', insertImage('../game/images/scene/lebre.png')]
        );
        await pool.query(
          'INSERT INTO Scene (scene_id, name, image) VALUES ($1, $2, $3)',
          [6, 'Absolem', insertImage('../game/images/scene/lagarta.png')]
        );
        await pool.query(
          'INSERT INTO Scene (scene_id, name, image) VALUES ($1, $2, $3)',
          [7, 'Tweedledee e Tweedledum', insertImage('../game/images/scene/irmaos.png')]
        );
    
    console.log('Populando tabela Dialogue');
    ...
    console.log('Populando tabela Item');
    ...
    console.log('Populando tabela Card');
    ...
    console.log('Populando tabela Deck');
      await pool.query(`
